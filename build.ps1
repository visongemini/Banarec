...
$legacyOutputs = @('Banarec.exe','Banarec.exe.config','Banarec.ico','Banarec.png')
foreach ($name in $legacyOutputs) { Remove-Item (Join-Path $OutputDirectory $name) -Force -ErrorAction SilentlyContinue }
...
$refs = @('System.Drawing.dll','System.Windows.Forms.dll','System.Xaml.dll',
    ...
    "$framework\WPF\PresentationFramework.dll", "$framework\WPF\UIAutomationClient.dll",
    "$framework\WPF\UIAutomationTypes.dll", "$OutputDirectory\ScreenRecorderLib.dll")
$compilerArgs = @('/nologo','/target:winexe','/platform:x64','/optimize+',
    "/win32manifest:$root\src\app.manifest", "/win32icon:$root\assets\BanaStudio.ico",
    "/out:$OutputDirectory\BanaStudio.exe", "/resource:$root\src\Main.xaml,Main.xaml")
...
Copy-Item "$root\assets\BanaStudio.ico","$root\assets\BanaStudio.png" $OutputDirectory -Force
Copy-Item "$root\src\BanaStudio.exe.config" $OutputDirectory -Force
...
Write-Host "Built $OutputDirectory\BanaStudio.exe"
