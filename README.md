...
1. 在 [Releases](https://github.com/visongemini/Banarec/releases/latest) 下载 `Banarec-Setup-2.2.0.exe`。
...
表中是默认值。进入**偏好设置**，点击截图或录屏快捷键，直接按下新的组合键即可修改；支持包含 Ctrl、Alt 或 Shift 的组合键，也可一键恢复默认。

当其他程序已注册所选组合键时，Banarec 会启用兼容方式响应。Ctrl + A 等未设置的组合键保持原有行为。

该兼容机制只判断已设置的两个组合键，不收集、保存或传输键入内容。
...
输出为 `dist\Banarec-Setup-2.2.0.exe`。安装包以当前用户权限安装，不需要管理员服务。
